...
public class SalesSystemUI extends JFrame {
	...
	private void drawWidgets() {
		...
		tabbedPane.addChangeListener(new ChangeListener() {
			@Override
			// Latest state from the database is loaded whenever a tab is selected
			public void stateChanged(ChangeEvent arg0) {
			if (tabbedPane.getSelectedIndex() == 0) {
				log.info("Purchase tab selected");
			}
			else if (tabbedPane.getSelectedIndex() == 1) {
				log.info("Stock tab selected");
				stockTab.refresh();
			}
			else if (tabbedPane.getSelectedIndex() == 2) {
				log.info("History tab selected");
				historyTab.refresh();
			}
			else if (tabbedPane.getSelectedIndex() == 3) {
				log.info("Client tab selected");
				clientTab.refresh();
			}
			}
	    });
